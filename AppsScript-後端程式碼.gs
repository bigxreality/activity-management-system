 ...
 *    - 類型選「網頁應用程式」
 *    - 執行身份：**我**（就是建立這份 Sheet 的那個帳號，例如 Eric）
 *    - 具有存取權的使用者：**知道連結的使用者（限 Google 帳戶）**
 *    - 按「部署」，複製產生的網址（結尾是 /exec）
 *    - 同事第一次打開網頁應用程式網址時，Google 會要求先用 Google 帳號登入，
 *      屬正常現象
 ...
 * 網址才會套用最新程式碼。
 *
 * 為什麼「執行身份」要選「我」，不是「使用者本人」：
 * - 「使用者本人」模式下，程式碼會用「打開網頁的那個人」的身份執行，這代表
 *   除了「使用者權限」分頁登記名字之外，每個人還要另外被加到這份 Google Sheet
 *   的共用清單裡，才能讀寫成功，等於要維護兩份權限清單，容易漏掉、也容易搞混。
 * - 「我」模式下，程式碼永遠用 Sheet 擁有者的身份執行，讀寫不會被卡；只要
 *   「具有存取權的使用者」設成「知道連結的使用者（限 Google 帳戶）」，Google
 *   還是會強制訪客先登入，所以 Session.getActiveUser().getEmail() 一樣能正確
 *   拿到訪客的信箱——效果相同，但只要維護「使用者權限」這一份清單就好。
 *
 * 權限系統：
